...
bool VariableManager::setupVariables(const vector<VarDef>& variables, TTree* tree) {
...
bool VariableManager::processEvent(const vector<VarDef>& variables) {
    for (const auto& varDef : variables) {
        ...
        switch (varDef.type) {
            case VarType::FLOAT: {
                auto* realVar = dynamic_cast<RooRealVar*>(it->second);
                if (realVar) {
            ...
            case VarType::BOOL: {
                auto* catVar = dynamic_cast<RooCategory*>(it->second);
                if (catVar) {
...
bool VariableManager::setupBasicVariable(const VarDef& varDef, TTree* tree) {
    switch (varDef.type) {
        case VarType::FLOAT: {
            auto* var = new RooRealVar(varDef.name.c_str(), varDef.name.c_str(),
        ...
        case VarType::BOOL: {
            auto* var = new RooCategory(varDef.name.c_str(), varDef.name.c_str());
...
bool VariableManager::setupFormulaVariables() {
    for (const auto& varDef : formulaVars) {
        ...
        auto* var = new RooFormulaVar(varDef.name.c_str(),
...
vector<RooDataSet*> RDSMaker(
    ...
    auto* baseDataset = new RooDataSet("dataset", "Base Dataset", varMgr.baseVarSet);
    ...
    for (Long64_t entry = 0; entry < nEntries; entry++) {
        ...
        if (saveHX || saveCS || saveEP) {
           ...
           if (totalProcessed < 5 || totalProcessed % 1000000 == 0) {
    cout << "[EP DEBUG] processed " << totalProcessed
         << " (entry " << entry << ") cosThetaEP: "
    ...
    if (!varMgr.formulaVars.empty()) {
        ...
        for (const auto& varDef : varMgr.formulaVars) {
            ...
            if (it != varMgr.varMap.end()) {
                auto* formulaVar = dynamic_cast<RooFormulaVar*>(it->second);
                if (formulaVar) {
...
// ==================== Enhanced Interface Helpers ====================

namespace {

const string DEFAULT_INPUT_FILE =
    "/home/jun502s/DstarAna/DStarAnalysis/Macro/skim/Data/FlatSample/ppData/"
    "flatSkimForBDT_DStar_ppRef_NonSwapData_PbPb_Data_ONNX_0_14Apr25.root";
...
vector<VarDef> buildDefaultVariableList(const DStarRDSOptions& options) {
...
DatasetCatalogEntry loadDatasetEntry(const DStarRDSOptions& options) {
    if (options.datasetKey.empty()) {
        throw runtime_error("Dataset key is empty while attempting to load catalog entry.");
    }

    DatasetCatalogEntry entry;
    auto info = JsonLoader::LoadObjectMap(options.datasetCatalog, "datasets", options.datasetKey);

    auto fileIt = info.find("file");
    if (fileIt == info.end()) {
        throw runtime_error("Dataset entry for key '" + options.datasetKey + "' is missing required 'file' field.");
    }
    entry.file = fileIt->second;

    auto treeIt = info.find("dataset_name");
    if (treeIt != info.end()) {
        entry.treeName = treeIt->second;
    }

    auto descIt = info.find("description");
    if (descIt != info.end()) {
        entry.description = descIt->second;
    }

    return entry;
}

string resolveInputPath(const DStarRDSOptions& options, const DatasetCatalogEntry* entry = nullptr) {
    if (!options.inputPath.empty()) {
        return options.inputPath;
    }

    if (entry && !entry->file.empty()) {
        return entry->file;
    }

    if (!options.datasetKey.empty()) {
        throw runtime_error("Dataset key '" + options.datasetKey + "' provided but failed to resolve input file.");
    }

    cout << ">>> 입력 파일 경로가 지정되지 않아 기본 경로를 사용합니다." << endl;
    return DEFAULT_INPUT_FILE;
}

string buildOutputFilename(const DStarRDSOptions& options) {
    string outputfilename = options.isMC ? "RDS_Physics_MC" : "RDS_Physics_Data";
    outputfilename += options.isD0 ? "_D0" : "_DStar";
    outputfilename += options.isPP ? "_ppRef" : "_PbPb";
    if (!options.suffix.empty()) {
        outputfilename += "_" + options.suffix;
    }
    outputfilename += ".root";
    return outputfilename;
}

string resolveOutputDirectory(const DStarRDSOptions& options) {
    const string& dir = options.isPP ? options.outputDirPP : options.outputDirPbPb;
    if (dir.empty()) {
        throw runtime_error("Output directory is not configured for the selected collision system.");
    }
...
void RunDStarRDSMaker(const DStarRDSOptions& options) {
    cout << "\n========================================" << endl;
    cout << "  D* RDS Maker (Enhanced Interface)" << endl;
    cout << "========================================" << endl;
    cout << "Configuration:" << endl;
    cout << "  - MC: " << (options.isMC ? "Yes" : "No") << endl;
    cout << "  - Particle: " << (options.isD0 ? "D0" : "D*") << endl;
    cout << "  - Collision: " << (options.isPP ? "pp" : "PbPb") << endl;
    cout << "  - Max entries: " << (options.maxEntries < 0 ? string("All") : to_string(options.maxEntries)) << endl;
    cout << "========================================\n" << endl;

    vector<VarDef> variables = buildDefaultVariableList(options);
    DatasetCatalogEntry datasetEntry;
    bool hasDatasetEntry = false;

    if (!options.datasetKey.empty()) {
        try {
            datasetEntry = loadDatasetEntry(options);
            hasDatasetEntry = true;
            cout << "  - Dataset key: " << options.datasetKey << endl;
            if (!datasetEntry.description.empty()) {
                cout << "  - Description: " << datasetEntry.description << endl;
            }
            cout << "  - Source file: " << datasetEntry.file << endl;
            if (!datasetEntry.treeName.empty()) {
                cout << "  - Tree name: " << datasetEntry.treeName << endl;
            }
            cout << "----------------------------------------" << endl;
    ...
    try {
        inputfilename = resolveInputPath(options, hasDatasetEntry ? &datasetEntry : nullptr);
    } catch (const exception& e) {
        cerr << "ERROR: " << e.what() << endl;
        return;
    }
    ...
    if (hasDatasetEntry && !datasetEntry.treeName.empty()) {
        treename = datasetEntry.treeName;
    }

    bool saveCS = options.saveCS;
    bool saveHX = options.saveHX && !options.isD0;
    bool saveEP = options.saveEP && !options.isD0;

    if (options.isD0) {
        if (options.saveHX) {
            cout << ">>> D0 모드에서는 Helicity 프레임이 비활성화됩니다." << endl;
        }
        if (options.saveEP) {
            cout << ">>> D0 모드에서는 Event Plane 프레임이 비활성화됩니다." << endl;
        }
    }

    string effMapName = options.effMapName.empty() ? "efficiency_map" : options.effMapName;
    ...
    string outputfilename = buildOutputFilename(options);
...
void DStarRDSMakerImproved(
    ...
    const string& datasetCatalog
) {
    DStarRDSOptions options;
    options.isMC = isMC;
    options.isD0 = isD0;
    options.isPP = isPP;
    options.inputPath = inputPath;
    options.suffix = suffix;
    options.effFile = effFile;
    options.effMapName = effMapNameInFile;
    options.saveCS = false;
    options.saveHX = !isD0;
    options.saveEP = !isD0;
    options.datasetKey = datasetKey;
    options.datasetCatalog = datasetCatalog.empty() ? options.datasetCatalog : datasetCatalog;

    RunDStarRDSMaker(options);
}
