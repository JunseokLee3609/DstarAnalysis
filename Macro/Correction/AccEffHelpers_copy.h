...
const char* TEfficiencyEndName(const char* refFrameName = "CS", Int_t ptMin = 0, Int_t ptMax = 30, Float_t rho_00=0) {
	return Form("%s%s_pt%dto%d_%s", refFrameName, gDstarAccName, ptMin, ptMax, PolaWeightName(rho_00));
}
const char* TEfficiencyPtEndName(const char* refFrameName = "CS", Int_t CosThetaMin = gCosThetaMin, Int_t CosThetaMax = gCosThetaMax, Float_t rho_00=0) {
	return Form("%s%s_CosTheta%dto%d_%s", refFrameName, gDstarAccName, CosThetaMin, gCosThetaMin, PolaWeightName(rho_00));
}

const char* TEfficiencyMainTitle(const char* title = "total efficiency") {
	return Form("D^{*}  %s", title);
}

const char* TEfficiencyAccMainTitle() {
	return Form("D^{*} acceptance");
}
/// TEfficiency constructors

// cos theta distribution for a given pT range
TEfficiency* CosThetaTEfficiency1D(const char* refFrameName = "CS", Int_t ptMin = 0, Int_t ptMax = 30, Bool_t isAcc = kFALSE, Float_t rho_00=0) {
	const char* name = Form("CosTheta%s", TEfficiencyEndName(refFrameName, ptMin, ptMax, rho_00));

	char* title = nullptr;
	if (isAcc)
		title = Form(";%s;%s", CosThetaVarTitle(refFrameName), TEfficiencyAccMainTitle());
	else
		title = Form(";%s;%s", CosThetaVarTitle(refFrameName), TEfficiencyMainTitle());
...
TEfficiency* PtTEfficiency1D(const char* refFrameName = "CS", Int_t CosThetaMin = gCosThetaMin, Int_t CosThetaMax = gCosThetaMax, Bool_t isAcc = kFALSE, Float_t rho_00=0) {
	const char* name = Form("CosTheta%s", TEfficiencyPtEndName(refFrameName, CosThetaMin, CosThetaMax, rho_00));

	char* title = nullptr;
	if (isAcc)
		title = Form(";%s;%s", PtVarTitle(refFrameName), TEfficiencyAccMainTitle());
	else
		title = Form(";%s;%s", PtVarTitle(refFrameName), TEfficiencyMainTitle());

	TEfficiency* tEff = nullptr;
	if (std::string(refFrameName) == "CS")
		tEff = new TEfficiency(name, title, NPtBinsCS, PtBinningCS);
	else
		tEff = new TEfficiency(name, title, NPtBinsHX, PtBinningHX);
...
// const char* RelativeSystTEfficiency2DName(const char* refFrameName = "CS", Int_t ptMin = 0, Int_t ptMax = 30, Float_t rho_00 = 0) {
// 	return Form("RelativeSyst2D%s", TEfficiencyEndName(refFrameName, ptMin, ptMax, rho_00));
// }

const char* CosThetaPtTEfficiency2DName(const char* refFrameName = "CS", Float_t rho_00 = 0) {
	return Form("CosThetaPt%s", TEfficiencyEndName(refFrameName, rho_00));
}

TEfficiency* CosThetaPtTEfficiency2D(const char* refFrameName = "CS", Int_t ptMin = 0, Int_t ptMax = 30,Float_t rho_00=0) {
	const char* name = CosThetaPtTEfficiency2DName(refFrameName, ptMin, ptMax, rho_00);

	const char* title = Form(";%s;%s;%s", CosThetaVarTitle(refFrameName), PhiAxisTitle(refFrameName), TEfficiencyMainTitle());

	TEfficiency* tEff = new TEfficiency(name, title, NCosThetaBins, gCosThetaMin, gCosThetaMax, NPtBins, gPtMin, gPtMax);
...
TEfficiency* CosThetaPtAcceptance2D(const char* refFrameName = "CS", Int_t ptMin = 0, Int_t ptMax = 30,Float_t rho_00=0) {
	const char* name = CosThetaPtTEfficiency2DName(refFrameName, ptMin, ptMax, rho_00);

	const char* title = Form(";%s;%s;acceptance", CosThetaVarTitle(refFrameName), PtAxisTitle(refFrameName));
...
// const char* NominalTEfficiency3DName(const char* refFrameName = "CS", Float_t rho_00=0) {
// 	return Form("Nominal3D%s%s_%s", refFrameName, gDstarAccName, PolaWeightName(rho_00));
// }

// const char* RelativeSystTEfficiency3DName(const char* refFrameName = "CS", Float_t rho_00= 0) {
// 	return Form("RelativeSyst3D%s%s_%s", refFrameName, gDstarAccName, PolaWeightName(rho_00));
// }
...
// const char* TEfficiency3DTitle(const char* refFrameName = "CS") {
// 	return Form("%s;%s", TEfficiencyMainTitle(), TEfficiency3DAxisTitle(refFrameName));
// }

// TEfficiency* TEfficiency3D(const char* name, const char* refFrameName = "CS") {
...
// 	TEfficiency* tEff = new TEfficiency(name, TEfficiency3DTitle(refFrameName), nCosThetaUltraFineBinning, cosThetaBinEdges.data(), nPhiUltraFineBinning, phiBinEdges.data(), NPtFineBinning, gPtFineBinning); // variable size binning for the stats
...
void DrawEfficiency1DHist(TEfficiency* effHist,TString var = "pT", Bool_t isAcc = kTRUE,  Float_t rho_00=0) {
	...
	if (var=="CosTheta") {
		...
		frameHist = new TH1D("frameHist", "", NPtFineBinning, gPtFineBinning);
	}
	...
	legend.DrawLatexNDC(.55, .8, Form("D^{*} for |#eta^{#mu}| < 2.4, %s", gDstarPtCutText));
	...
	else frameHist->SetXTitle("pT (GeV/c)");

	if (isAcc)
		frameHist->SetYTitle(TEfficiencyMainTitle("Acceptance"));
	else
		frameHist->SetYTitle(TEfficiencyMainTitle());
	...
	if (isAcc) {
		...
		canvas->SaveAs(Form("AcceptanceMaps/Dstar/%s.png", effHist->GetName()), "RECREATE");
	} else {
		gSystem->mkdir(Form("EfficiencyMaps/Dstar"), kTRUE);
		canvas->SaveAs(Form("EfficiencyMaps/Dstar/%s.png", effHist->GetName()), "RECREATE");
	}
}


void DrawEfficiency2DHist(TEfficiency* effHist, Int_t ptMin, Int_t ptMax, Int_t nCosThetaBins = 5, const std::vector<Double_t>& cosThetaBinEdges = {}, Int_t nPhiBins = 5, const std::vector<Double_t>& phiBinEdges = {}, Bool_t isAcc = kTRUE, Bool_t displayValues = kFALSE, const char* extraString = "") {
	...
	legend.DrawLatexNDC(.48, .87, Form("%s < 2.4, %s", gDimuonRapidityVarTitle, DimuonPtRangeText(ptMin, ptMax)));
	legend.DrawLatexNDC(.48, .83, Form("D^{*} acc. for |#eta^{#mu}| < 2.4, %s", gDstarPtCutText));
	...
	if (isAcc) {
		effHist->SetTitle(Form(";;;%s", TEfficiencyMainTitle("Acceptance")));
		SetColorPalette(gAcceptanceColorPaletteName);
	} else {
		effHist->SetTitle(Form(";;;%s", TEfficiencyMainTitle()));
	...
	gSystem->mkdir(Form("EfficiencyMaps/Dstar"), kTRUE);
	if (isAcc)
		canvas->SaveAs(Form("EfficiencyMaps/Dstar/2Dacc_%s_pt%dto%d.png", effHist->GetName(), ptMin, ptMax), "RECREATE");
	else
		canvas->SaveAs(Form("EfficiencyMaps/Dstar/2Deff_%s_pt%dto%d.png", effHist->GetName(), ptMin, ptMax), "RECREATE");
}