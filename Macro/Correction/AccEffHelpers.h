...
void DrawEfficiency1DHist(TEfficiency* effHist,TString var = "pT", Bool_t isAcc = kTRUE,  Float_t lambdaTheta = 0, Float_t lambdaPhi = 0, Float_t lambdaThetaPhi = 0,Int_t iState=1) {
	...
	legend.DrawLatexNDC(.55, .8, Form("D^{*} for |#eta^{#mu}| < 2.4, %s", gMuonPtCutText));
...
void DrawEfficiency2DHist(TEfficiency* effHist, Int_t ptMin, Int_t ptMax, Int_t nCosThetaBins = 5, const std::vector<Double_t>& cosThetaBinEdges = {}, Int_t nPhiBins = 5, const std::vector<Double_t>& phiBinEdges = {}, Int_t iState = gUpsilonState, Bool_t isAcc = kTRUE, Bool_t displayValues = kFALSE, const char* extraString = "") {
	...
	legend.DrawLatexNDC(.48, .83, Form("D^{*} acc. for |#eta^{#mu}| < 2.4, %s", gMuonPtCutText));
