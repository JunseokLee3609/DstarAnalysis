...
void test2(
    ...
    auto fillGEN = [&](int i){
            ...
            if(fabs(evtMC.gen_pT[i]-Dstar.Pt()) > 0.01) std::cout << "Dstar pT: " << Dstar.Pt() << " D0 pT: " << evtMC.gen_pT[i] << std::endl;
            if(fabs(evtMC.gen_y[i]-Dstar.Rapidity()) > 0.01) std::cout << "Dstar y: " << Dstar.Rapidity() << " D0 y: " << evtMC.gen_y[i] << std::endl;
