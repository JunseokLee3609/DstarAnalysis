...
void TreeTest(
    ...
    // string fileName="output_pthat10.root"
){
    // Open MC file
    TFile* f = TFile::Open(fileName.c_str());
    // TFile* feffMap = TFile::Open("output_nonprompt_onlygen.root");
    ...
    for (auto idx : ROOT::TSeqU(nEvts)) {
        ...
        for (auto i : ROOT::TSeqI(evtMC->candSize_gen)) {
            ...
            // TVector3 vect = DstarDau1Vector_EventPlane(Dstar, D0, evtMC->Psi2Raw_Trk[i]);
            cout << "psiangle " <<evtMC->Psi2Raw_Trk[i] << endl; 
            double cosTheta = vect.CosTheta();
            
            // Determine prompt/non-prompt based on ancestor flavor (same as FlatTest.cpp)
            bool isPrompt = (evtMC->gen_D0ancestorFlavor_[i] != 5); // 5 = b-quark, non-prompt if from b-quark
