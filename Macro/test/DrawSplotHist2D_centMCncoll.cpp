...
std::vector<double> CalSplotweight(FitOpt D0Opt, TH3D* hist_eff, double mvaCut, std::string subDir){
    ...
    for( auto idx: ROOT::TSeqI(wds_sig->numEntries())){
        ...
        if (fabs(sWeight_val) > 1e-9 && fabs(corr_) > 1e-9) {
             corr_ /= sWeight_val*ncoll->getVal();
             corr2 += 1.0/corr_;
             count2+= sWeight_val;
        }
...
void DrawSplotHist2D_centMCncoll(){
    // TFile *effFile = TFile::Open("/home/jun502s/DstarAna/DStarAnalysis/Macro/Correction/output_MC_May09_noNcoll.root","read");
    TFile *effFile = TFile::Open("/home/jun502s/DstarAna/DStarAnalysis/Macro/Correction/output_MC_May17_ncoll_v1.root","read");
    std::string subDir = "D0_May17_MC_pol1_v1/";
    ...
    std::vector<double> mvaBin= {0.90,0.91,0.92,0.93,0.94,0.95,0.96,0.97,0.98,0.990, 0.991, 0.992, 0.993, 0.994, 0.995, 0.996, 0.997, 0.998, 0.999};
    ...
    for(size_t icent = 0; icent < centBins.size(); icent++) {
        ...
        for( auto idx : ROOT::TSeqUL(numGenEntries)){ // Use TSeqUL for unsigned long long
            ...
            // int icent = 0;
            if(evtGenFlat.centrality/2 < centBins[icent].first || evtGenFlat.centrality/2 >= centBins[icent].second || evtGenFlat.gen_pT<2 || evtGenFlat.gen_pT> 50 ) continue;
            counts += evtGenFlat.ncoll;
                // icent++;
    ...
    for(size_t icent = 0; icent < centBins.size(); icent++) {
        ...
        c_gen->SetGrid(1,1);
        c_gen->SaveAs(Form("splotMCncoll/splot_gen_yield_%s_%s.png",subStr.c_str(),centLabel.c_str()));
        ...
        leg1->Draw("same");
        c_raw->SetGrid(1,1);
        c_raw->SaveAs(Form("splotMCncoll/plot_raw_yield_cent_%s_%s.png",subStr.c_str(),centLabel.c_str()));
        delete leg1;
        ...
        c_yield->SaveAs(Form("splotMCncoll/splot_corrected_yield_cent_%s_%s.png",subStr.c_str(),centLabel.c_str()));
        // delete c_yield;
        ...
        c_yield_cb->SaveAs(Form("splotMCncoll/splot_corrected_yield_cent_cb_%s_%s.png",subStr.c_str(),centLabel.c_str()));
        ...
        c_yield_gm->SaveAs(Form("splotMCncoll/splot_corrected_yield_cent_gm_%s_%s.png",subStr.c_str(),centLabel.c_str()));
        ...
        c_weight->SaveAs(Form("splotMCncoll/splot_weight_cent_%s_%s.png",subStr.c_str(),centLabel.c_str()));
        ...
        c_weight_cb->SaveAs(Form("splotMCncoll/splot_weight_cent_cb_%s_%s.png",subStr.c_str(),centLabel.c_str()));
        ...
        c_weight_gm->SaveAs(Form("splotMCncoll/splot_weight_cent_gm_%s_%s.png",subStr.c_str(),centLabel.c_str()));
        ...
        h_raw_yields[icent]->Scale(h_corrected_yields_gm[icent]->GetBinContent(1)/h_raw_yields[icent]->GetBinContent(1));
        h_raw_yields[icent]->SetLineColor(kRed);
        h_raw_yields[icent]->SetMarkerColor(kRed);
        h_raw_yields[icent]->SetMarkerStyle(24);
        h_raw_yields[icent]->SetMarkerSize(1.0);
        ...
        h_raw_yields_gm[icent]->SetMarkerStyle(24);
        h_raw_yields_gm[icent]->SetMarkerSize(1.0);
        h_raw_yields_gm[icent]->Scale(h_corrected_yields_gm[icent]->GetBinContent(1)/h_raw_yields_gm[icent]->GetBinContent(1));
        ...
        c_comp->SaveAs(Form("splotMCncoll/splot_yield_comparison_cent_%s_%s.png",subStr.c_str(),centLabel.c_str()));
        // TFile *outputFile = new TFile(("splot_yield_comparison_cent_MC_ncoll" + centLabel + ".root").c_str(), "RECREATE");
        TFile *outputFile = new TFile(Form("splotMCncoll/splot_yield_comparison_cent_%s_%s.root",subStr.c_str(),centLabel.c_str()), "RECREATE");
