...
std::vector<double> CalSplotweight(FitOpt D0Opt, TH3D* hist_eff, double mvaCut, std::string subDir){
    ...
    cout << count2 << endl;
    return std::vector<double> {corr2/count2, sumInvEff / countValidEff};
...
void DrawSplotHist2D_centData(){
    ...
    std::string subDir = "D0_May09_Data_v1/";
        ...
        for (TH1D* h : hists_to_label) {
            ...
            for (int bin_idx = 1; bin_idx <= xAxis->GetNbins(); ++bin_idx) {
                ...
                if (bin_idx -1 < mvaBinEdges.size() && bin_idx < mvaBinEdges.size()) {
                    ...
                    xAxis->ChangeLabel(bin_idx, 315., -1., -1, -1, -1, binLabel);
    ...
    for(size_t icent = 0; icent < centBins.size(); icent++) {
        ...
        c_yield->SetGrid(1,1);
        c_yield->SaveAs(("splot_corrected_yield_cent_Data" + centLabel + ".png").c_str());
        // delete c_yield;
        ...
        c_yield_cb->SaveAs(("splot_corrected_yield_cent_cb_Data" + centLabel + ".png").c_str());
        ...
        c_weight->SaveAs(("splot_weight_cent_Data" + centLabel + ".png").c_str());
        // delete c_weight;
        ...
        c_weight_cb->SaveAs(("splot_weight_cent_cb_Data" + centLabel + ".png").c_str());
        ...
        leg->SetBorderSize(0);
        leg->AddEntry(h_corrected_yields[icent], "Corrected Yield, Splot", "pe");
        ...
        c_comp->Update();
        c_comp->SetGrid(1,1);
        c_comp->SaveAs(("splot_yield_comparison_cent_Data" + centLabel + ".png").c_str());
        delete leg;
        delete c_comp;
    }
