...
void DrawSplotHist2D_ptMC(){
    ...
    for(size_t ipt = 0; ipt < ptBins.size(); ipt++) {
        ...
        for( auto idx : ROOT::TSeqUL(numGenEntries)){ // Use TSeqUL for unsigned long long
            ...
            // int ipt = 0;
            if(evtGenFlat.centrality/2 < centBins[0].first || evtGenFlat.centrality/2 >= centBins[0].second || evtGenFlat.gen_pT< ptBins[ipt].first || evtGenFlat.gen_pT> ptBins[ipt].second ||abs(evtGenFlat.gen_y)>1) continue;
            counts ++;
                // ipt++;
