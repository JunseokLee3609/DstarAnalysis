...
void DrawSplotHist2D_pt(){
    ...
    TH2D *h_raw_yield_2d = (TH2D*)yieldFile->Get("D0_Yield_mva_pt");
     if (!h_raw_yield_2d) {
    ...
    for(size_t icent = 0; icent < ptBins.size(); ++icent) {
        std::string centLabel = std::to_string((int)ptBins[icent].first) + "to" + std::to_string((int)ptBins[icent].second);
    ...
    for(size_t imva = 0; imva < mvaBinEdges.size() - 1; ++imva) {
         ...
         int icent =0;
            ...
            for(auto ptbin : ptBins){
                ...
                cout << "cent " << ptbin.first << " to " << ptbin.second << " mva " << mva_cut << " corr " << corr << endl;
    ...
    for(size_t ipt = 0; ipt < ptBins.size(); ++ipt) {
        ...
        c_yield->SetGrid(1,1);
        c_yield->SaveAs(("splot_corrected_yield_pt_noncoll_" + ptLabel + ".png").c_str());
        delete c_yield;
        ...
        c_weight->SaveAs(("splot_weight_pt_noncoll_" + ptLabel + ".png").c_str());
        delete c_weight;
        ...
        latex.DrawLatex(0.25, 0.85, Form("pt = %.1f to %.1f", ptBins[ipt].first, ptBins[ipt].second));
        c_comp->SetGrid(1,1);
        c_comp->SaveAs(("splot_yield_comparison_pt_noncoll_" + ptLabel + ".png").c_str());
        delete leg;
        delete c_comp;
    }
