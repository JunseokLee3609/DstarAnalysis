...
void DrawSplotHist2D_ptMC(){
    ...
    std::string subDir = "D0_May14_MC_pol3_v1/";
    std::string subStr = "bkgpol3";
    ...
    for(size_t ipt = 0; ipt < ptBins.size(); ipt++) {
        ...
        for( auto idx : ROOT::TSeqUL(numGenEntries)){ // Use TSeqUL for unsigned long long
            ...
            // int ipt = 0;
            if(evtGenFlat.centrality/2 < centBins[0].first || evtGenFlat.centrality/2 >= centBins[0].second || evtGenFlat.gen_pT< ptBins[ipt].first || evtGenFlat.gen_pT> ptBins[ipt].second ||abs(evtGenFlat.y)>1) continue;
            counts ++;
                // ipt++;
    ...
    std::vector<double> mvaBinpters;
    for(size_t i = 0; i < mvaBinEdges.size() - 1; ++i) {
        mvaBinpters.push_back( (mvaBinEdges[i] + mvaBinEdges[i+1]) / 2.0 );
    }
    ...
    if (!effmap) {
        ...
        return;
            ...
            for(auto ptbin : ptBins){
                ...
                // if(ptbin.first == 0 && ptbin.second == 90) {
                //     h_raw_yield_2d=(TH2D*)yieldFile->Get("D0_Yield_mva_pt");
                // } 
                // else{
                //     h_raw_yield_2d=(TH2D*)yieldFile->Get("D0_Yield_mva_pt");
                // }
