...
std::unique_ptr<RooFitResult> MassFitter::fixFit(RooFitResult* initialResult) {
    ...
    while(needRefit) {
        ...
        for(int idx = 0; idx < fpf.getSize(); ++idx) {
            ...
            if( _val_ + 3 * _errup_ > _limup_ || (_val_ > 0 && 1.2 * _val_ > _limup_) || (_val_ < 0 && 0.8 * _val_ > _limup_)) {
                ...
                // if(_vname_.find("n") != std::string::npos){cout << "skip upper limit for n" << std::endl; continue;}
                if(_vname_.find("m0") != std::string::npos) {cout << "skip exapnding upper limit for m0" << std::endl; continue; }
            ...
            if(_val_ + 3 * _errdo_ < _limdown_ || (_val_ > 0 && 0.8 * _val_ < _limdown_) || (_val_ < 0 && 1.2 * _val_ < _limdown_)) {
                ...
                if(_vname_.find("m0") != std::string::npos) {cout << "skip exapnding lower limit for m0" << std::endl; continue; }
                ...
                // }
                if(_vname_.find("n") != std::string::npos){
                    std::cout << "skip lower limit for n" << std::endl; 
                }
    ...
    while(tempResult->status() !=0 && (ntry+1)<3){
        ...
        needRefit = ((tempResult->statusCodeHistory(0) != 0) || (tempResult->statusCodeHistory(1) != 0));
