...
#include "../ErrorHandler.h"            // Unified logging
...
void DStarAnalysisV2forpp(bool doReFit = false, bool doDCA = true, bool plotFit = true, bool useCUDA = true,
    ...
    ErrorHandlerManager::Instance().LogInfo("=== D* Meson Analysis V2 ===", "DStarAnalysisV2forpp");
    ...
    // config.SetDataFilePath("/home/jun502s/DstarAna/DStarAnalysis/Data/RDS_Physics/RDS_Physics_Data_DStar_ppRef_ppPU_Prompt_20Sep25_v1.root");
    ...
    // config.SetDataFilePath("/home/jun502s/DstarAna/DStarAnalysis/Data/RDS_MC/RDS_Physics_MC_DStar_ppRef_ppPU_np_16Sep25_v1.root");
//    config.SetDataFilePath("/home/jun502s/DstarAna/DStarAnalysis/Data/RDS_MC/RDS_Physics_MC_DStar_ppRef_ppPU_NoCorrection_20Sep25_v1.root");
    ...
    // config.SetMCFilePath("/home/jun502s/DstarAna/DStarAnalysis/Data/RDS_MC/RDS_Physics_MC_DStar_ppRef_ppPU_Prompt_20Sep25_v1.root");
    ...
    try {
        ...
        // Ensure the cut expression does NOT include a centrality cut even if the bin name carries cent ranges.
        {
            ...
            cout << "isPP = " << (isPP ? "true" : "false") << std::endl;    
            if (isPP) {
        ...
        if (!dataset) {
                        ...
                        if (fitOpt.fitMethod == FitMethod::GaussianConstraint) {
                            ...
                            if (!mcLoader.loadRooDataSet(config.GetDatasetName())) {
                                ...
                                fitSuccess = fitter->PerformGaussianConstraintFitWithMCAndBkgSB(
                                    ...
                                    ppBinName);
                                if (!fitSuccess) {
                                    ErrorHandlerManager::Instance().LogWarning("[GC+SB] Combined constraints failed; fallback to signal-only GC.", "DStarAnalysisV2forpp");
                            ...
                            if (useSBPrefit) {
                                ...
                                ErrorHandlerManager::Instance().LogInfo("[SB] Attempting sideband prefit-constrained fit...", "DStarAnalysisV2forpp");
                                fitSuccess = fitter->PerformSidebandPrefitBackgroundConstraintFit(
                                    ...
                                    ppBinName);
                                if (!fitSuccess) {
                                    ErrorHandlerManager::Instance().LogWarning("[SB] Sideband-constrained fit failed; falling back to free fit.", "DStarAnalysisV2forpp");
                                }
                            }
                            if (!fitSuccess) {
            ...
            if (fitSuccess) {
                ErrorHandlerManager::Instance().LogInfo(std::string("Fit successful for bin: ") + ppBinName, "DStarAnalysisV2forpp");
                ...
                
                {
                    ...
                    ErrorHandlerManager::Instance().LogInfo(oss.str(), "DStarAnalysisV2forpp");
                }
                        ...
                        if (mcDataset) {
                            ...
                            if (mcFitSuccess) {
                                ...
                                try {
                                    ...
                                    EnhancedPlotManager mcPlotManager(fitOpt, mcOutDir, fitOpt.outputFile + ".root", mcPlotDir, true, true);
                                    if (mcPlotManager.IsValid()) {
                                        ...
                                        mcPlotManager.DrawFittedModel(true, "fitted_MC_noParamPad_" + ppBinName, false);
                                    }
                                } catch (const std::exception& e) {
                ...
                if (plotFit) {
                    ...
                    try {
                        ...
                        EnhancedPlotManager plotManager(fitOpt, inputDir, inputFile, plotDir, true, true);
                        ...
                        if (plotManager.IsValid()) {
                            ...
                            bool fitNoPadSuccess = plotManager.DrawFittedModel(true, "fitted_noParamPad_" + bin.GetBinName(), false);
                            if (fitNoPadSuccess) {
