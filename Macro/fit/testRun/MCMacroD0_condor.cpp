...
void MCMacroD0_condor(bool doReFit= false, bool plotFit = true, bool useCUDA=true, float pTMin=2, float pTMax=100, float centMin=0, float centMax=100, float mvaMin=0.9, std::string sub = "/D0_May09_Data_v1/") {
    ...
    //  std::vector<double> mvaBin = {0.990, 0.991, 0.992, 0.993, 0.994, 0.995, 0.996, 0.997, 0.998, 0.999};
     std::vector<double> mvaBin ={mvaMin};
    ...
    //  std::vector<double> mvaBin1 = {0.9,0.91,0.92,0.93,0.94,0.95,0.96,0.97,0.98,0.990, 0.991, 0.992, 0.993, 0.994, 0.995, 0.996, 0.997, 0.998, 0.999,1.000};
     std::vector<double> mvaBin1 = {0.990, 0.991, 0.992, 0.993, 0.994, 0.995, 0.996, 0.997, 0.998, 0.999,1.000};
    ...
    //std::string filepathMC = "/home/jun502s/DstarAna/DStarAnalysis/Data/RDS_MC/RDS_Physics_MC_PbPb_ONNX_withCent_wNcoll_May14.root";
    ...
    std::string filepathMC = "/home/jun502s/DstarAna/DStarAnalysis/Data/RDS_MC/RDS_Physics_MC_PbPb_ONNX_withCent_wNcoll_May17.root";
    ...
    //std::string filepathData = "/home/jun502s/DstarAna/DStarAnalysis/Data/RDS_MC/RDS_Physics_MC_PbPb_ONNX_withCent_wNcoll_May14.root";
    ...
    std::string filepathData = "/home/jun502s/DstarAna/DStarAnalysis/Data/RDS_MC/RDS_Physics_MC_PbPb_ONNX_withCent_wNcoll_May17.root";
    // std::string filepathData = "/home/jun502s/DstarAna/DStarAnalysis/Data/RDS_Physics/RDS_Physics_Data_PbPb_ONNX_withCent_mva0p7_Apr30_v1.root";
    ...
    auto params2 = D0BkgParamMaker({0},{0},3);
...
TH1D* h1D_SB = new TH1D("h1D_SB","D^{0} Signal/Background; MVA Score; S/B",
    mvaBin1.size()-1, &mvaBin1[0]); 
TH2D* h2D = new TH2D("h2D","D^{0} Yield; MVA Score", mvaBin1.size()-1, &mvaBin1[0],pTBin1.size()-1, &pTBin1[0]
);
...
int ipT=0;
    for(auto ptbin : ptBins){
	   ...
	   if(doReFit){
            ...
            //         h2D->SetBinContent(imva + 1, ipT +1, sigVal);
            //         h2D->SetBinError(imva + 1, ipT+1, sigErr);
            //         sigOk = true;
            //     } else {
            //          std::cout << "Warning: Could not retrieve signal yield TParameters from file: " << fullPath << std::endl;
            //          h2D->SetBinContent(imva + 1, ipT +1, 0);
            //          h2D->SetBinError(imva + 1, ipT+1, 0);
            //     }
            ...
            //             }
            //             h2D_SB->SetBinContent(imva + 1, ipT+1, sbRatio);
            //             h2D_SB->SetBinError(imva + 1, ipT+1, sbErr);
            //         } else {
            //             h2D_SB->SetBinContent(imva + 1,ipT+1, 0); // Set S/B to 0 if background is zero
            //             h2D_SB->SetBinError(imva + 1,ipT+1, 0);
            //         }
            //     } else {
            //         // If either signal or background failed, set S/B to 0
            //         h2D_SB->SetBinContent(imva + 1,ipT+1, 0);
            //         h2D_SB->SetBinError(imva + 1,ipT+1, 0);
            //     }
...
h2D->SetStats(0);
h2D->Draw("colztext");
c1->SaveAs("D0_Yield_mva.pdf");

// S/B 플롯 그리기 및 저장
TCanvas* c2 = new TCanvas("c2", "D0 S/B Ratio", 800, 600);
gPad->SetRightMargin(0.15);
h2D_SB->SetStats(0);
h2D_SB->Draw("colztext");
c2->SaveAs("D0_SB_Ratio_mva.pdf");


TFile *fout = new TFile("D0_Yield_mva.root", "RECREATE");
fout->cd();
