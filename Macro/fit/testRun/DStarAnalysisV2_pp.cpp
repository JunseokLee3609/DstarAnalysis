...
void DStarAnalysisV2_pp(bool doReFit = false, bool plotFit = true, bool useCUDA = true,
    ...
    config.SetYieldModeAutoTuning(true);
    ...
    config.SetUseIndependentYields(false); // choose Fraction; auto-tuning handles the rest
    ...
    try {
        ...
        if (!dataset) {
            ...
            if (doReFit) {
                ...
                // Build list of prefixes to fix/constraint from fixedFlags
                ...
                // Derive variable names for Gaussian constraints (data PDF uses suffix _signal)
                std::vector<std::string> dataParamNames;
                dataParamNames.reserve(prefixesToFix.size());
                for (const auto& p : prefixesToFix) dataParamNames.push_back(p + "_signal");
                        ...
                        if (fitOpt.fitMethod == FitMethod::GaussianConstraint) {
                            ...
                            if (!mcLoader.loadRooDataSet(config.GetDatasetName())) {
                                ...
                                fitSuccess = fitter->PerformGaussianConstraintFitWithMC(
                                    ...
                                    dataParamNames.empty() ? std::vector<std::string>{"mean_signal","sigma_signal"} : dataParamNames,
                                    bin.GetBinName());
                            }
                        } else if (fitOpt.fitMethod == FitMethod::FixedFromMC) {
                            // Load MC dataset, fit MC, then fix params and fit data
                            DataLoader mcLoader(config.GetMCFilePath());
                            if (!mcLoader.loadRooDataSet(config.GetDatasetName())) {
