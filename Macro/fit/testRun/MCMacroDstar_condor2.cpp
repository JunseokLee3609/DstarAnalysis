...
void MCMacroDstar_condor2(bool doReFit= false, bool plotFit = true,bool useCUDA=true, float pTMin=4, float pTMax=100, float cosMin=-2, float cosMax=2){
    FitOpt D0opt;
    ...
    std::string subDir = "/DStar_ppRef_MC_absCos_absEta_1p2_Jul17_test_v1/";
    std::string EtaCut = "abs(EtaD2)< 1.2 && abs(EtaGrandD1) < 1.2 && abs(EtaGrandD2) < 1.2 && pTD2>0.4";
    ...
    auto params2 = DStarParamMaker4({0},{0},2);
//         if(pTMin ==5 && pTMax == 7){
//         nsig_ratio = 0.3;        
//         nsig_min_ratio = 0.2;
//         nsig_max_ratio = 0.9;
