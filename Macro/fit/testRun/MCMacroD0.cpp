...
void MCMacroD0(bool doReFit= false, bool plotFit = true, bool useCUDA=true, float pTMin=2, float pTMax=100, float centMin=0, float centMax=100){
    std::string subDir = "/D0_May03_MCTest_inclusive/";
    FitOpt D0opt;
    ...
    std::vector<std::pair<double,double>> centBins = {
     ...
     std::vector<double> mvaBin = {0.7,0,8,0.9,0.91,0.92,0.93,0.94,0.95,0.96,0.97,0.98,0.990, 0.991, 0.992, 0.993, 0.994, 0.995, 0.996, 0.997, 0.998, 0.999};
...
TFile *fout = new TFile(Form("%sD0_Yield_mva_inclusive.root",D0opt.outputDir.c_str()), "RECREATE");
fout->cd();
