...
public:
    ...
    MassFitter(const std::string& name, std::string& massvar, double massMin, double massMax,
               double nsig_ratio = 0.01, double nsig_min_ratio = 0.0, double nsig_max_ratio = 1.0,
               double nbkg_ratio = 0.01, double nbkg_min_ratio = 0.0, double nbkg_max_ratio = 1.0);
    ~MassFitter();
    ...
    while(needRefit) {
        ...
        for(int idx = 0; idx < fpf.getSize(); ++idx) {
            ...
            if( _val_ + 3 * _errup_ > _limup_ || (_val_ > 0 && 1.2 * _val_ > _limup_) || (_val_ < 0 && 0.8 * _val_ > _limup_)) {
                ...
                if(_vname_.find("nsig") != std::string::npos) continue;
                if(_vname_.find("nbkg") != std::string::npos) continue;
                if(_vname_.find("alpha") != std::string::npos) continue;
                if(_vname_.find("n") != std::string::npos){pdfVar->setMax(pdfVar->getMax() * 2);} continue;
                if(_vname_.find("mean") != std::string::npos) continue;
                ...
                // if(_vname_.find("p0")!= std::string::npos) {
                    pdfVar->setMax(pdfVar->getMax() * 10);
            ...
            if(_val_ + 3 * _errdo_ < _limdown_ || (_val_ > 0 && 0.8 * _val_ < _limdown_) || (_val_ < 0 && 1.2 * _val_ < _limdown_)) {
                if(_vname_.find("sigma") != std::string::npos) continue; 
                if(_vname_.find("frac") != std::string::npos) continue;
                if(_vname_.find("nsig") != std::string::npos) continue;
                if(_vname_.find("nbkg") != std::string::npos) continue;
                if(_vname_.find("mean") != std::string::npos) continue;
                if(_vname_.find("p0")!= std::string::npos || _vname_.find("p1") != std::string::npos || _vname_.find("p2") != std::string::npos) {
                // if(_vname_.find("p0")!= std::string::npos) {
                    pdfVar->setMin(pdfVar->getMin() * 0.1);
                ...
                if(_vname_.find("alpha") != std::string::npos) continue;
                if(_vname_.find("n") != std::string::npos){pdfVar->setMin(pdfVar->getMin() * 0.9);} continue;
