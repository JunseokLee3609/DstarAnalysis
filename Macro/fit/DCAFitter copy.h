...
public:
    ...
    // --- Configuration ---
    void setMCFile(const std::string& fileName, const std::string& treeName, const std::string& datasetName = "") {
        mcFileName_ = fileName;
        mcTreeName_ = treeName;
        mcRooDatsetName_ = datasetName;
    }
    void setDataFile(const std::string& fileName, const std::string& treeName, const std::string& datasetName = "") {
    ...
    bool createTemplatesFromMC() {
        ...
        if(!mcRooDatsetName_.empty()) RooDataset* mcDataset = dynamic_cast<RooDataset*>(mcFile->Get(mcRooDatsetName_.c_str()));
        ...
        else fullMCDataSet.reset(new RooDataSet("fullMCDataSet", "Full MC DataSet", mcTree, mcVars, mcCuts_.c_str(), weightVarName));
    ...
    bool loadData() {
        ...
        if(!dataRooDatsetName_.empty()) RooDataset* dataDataset = dynamic_cast<RooDataset*>(dataFile->Get(dataRooDatsetName_.c_str()));
    ...
    void plotResults(RooFitResult* fitResult = nullptr, const std::string& plotName = "dca_fit_plot") {
        ...
        if (fracPrompt_ && fitResult) {
            ...
            latex.DrawLatex(0.55, 0.55, Form("Prompt frac. = %.1f #pm %.1f %%", frac_val*100, frac_err*100));
        }
...
inline void DCAFitter::plotRawDataDistribution(const std::string& plotName) {
    ...
    // latex.DrawLatex(0.5, 1 - canvas->GetTopMargin() + 0.02, "10.0 < p_{T} < 12.5 GeV/c");
