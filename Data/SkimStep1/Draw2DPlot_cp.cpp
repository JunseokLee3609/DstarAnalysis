...
void Draw2DPlot_cp(){
	gStyle->SetOptStat(0);
    TFile *f = new TFile("2Dhistogram_test.root","recreate");
...
std::cout << var << " -> nbin: " << std::get<0>(rangemap[var])
 ...
 std::vector<bool> select(vars.size(), false);
	...
	if((chosen[0].find("pTD1")!=std::string::npos&& chosen[1].find("pTD2")!=std::string::npos )|| ((chosen[0].find("EtaD1")!=std::string::npos && chosen[1].find("EtaD2")!=std::string::npos))){
		...
		ch1->Draw(Form("%s:%s>>%s",chosen[1].c_str(),chosen[0].c_str(),Form("hgen_%s",hist_name.c_str())),"matchGEN==1");
		ch1->Draw(Form("%s:%s>>%s",chosen[1].c_str(),chosen[0].c_str(),hist_name.c_str()));
		cout <<"Drawing" << endl;
        h1[i]->SetTitle("");
        hgen[i]->SetTitle("");
