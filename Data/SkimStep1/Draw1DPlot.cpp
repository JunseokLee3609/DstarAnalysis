...
void Draw1DPlot(){
	gStyle->SetOptStat(0);
    TFile *f = new TFile("1Dhistogram_test.root","recreate");
...
//	if(chosen[0].find("zDCASignificanceDaugther1")==std::string::npos) continue;
		...
		ch1->Draw(Form("pTD1/ pTerrD1>>%s", hist_name.c_str()));
		ch1->Draw(Form("pTD1 / pTerrD1>>hgen_%s", hist_name.c_str()), "matchGEN==1");
		}
		else if(chosen[0].find("pTSignificanceDaughter2")!=std::string::npos){
		ch1->Draw(Form("pTD2/ pTerrD2>>%s", hist_name.c_str()));
		ch1->Draw(Form("pTD2 / pTerrD2>>hgen_%s", hist_name.c_str()), "matchGEN==1");
		}
		else if(chosen[0].find("normalizedChi2")!=std::string::npos){
		ch1->Draw(Form("VtxChi2 / VtxNDF>>%s", hist_name.c_str()));
		ch1->Draw(Form("VtxChi2 / VtxNDF>>hgen_%s", hist_name.c_str()), "matchGEN==1");
		}
		else{
		ch1->Draw(Form("%s>>%s",chosen[0].c_str(),hist_name.c_str()));
		ch1->Draw(Form("%s>>hgen_%s",chosen[0].c_str(),hist_name.c_str()),"matchGEN==1");
		}
        	if (h1[i]->Integral() > 0) {
		...
		h1[i]->SetTitle("");
		hgen[i]->SetTitle("");
		leg[i]->AddEntry(h1[i],"background");
		leg[i]->AddEntry(hgen[i],"gen matching");
		...
		leg[i]->Draw("same");
		cvs[i]->SaveAs(Form("1DHisto/1DHist_%s.pdf",hist_name.c_str()));
    		h1[i]->Write(); 
		hgen[i]->Write();
...
c_combined->SaveAs(Form("1DHisto/CulmHist_%s.pdf", hist_name.c_str()));
