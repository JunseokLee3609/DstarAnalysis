#include "../../Common/EffHead.h"
#include "../../Common/commonSelectionVar.h"
...
// ANA::selBase selMVA,
// ANA::selBase selDCA,
// Previous manual file path examples retained for reference:
// - /home/jun502s/DstarAna/DStarAnalysis/Data/output_pbpb_mc_wgeninfo.root
// - /home/jun502s/DstarAna/DStarAnalysis/Data/MC/Dstar2024ppRef/Mar30NonSwap/d0ana_tree_nonswapsample_ppref_30Mar.root
// - /home/jun502s/DstarAna/DStarAnalysis/Macro/skim/Data/FlatSample/PbPbMC/DStar/0822/flatSkimForBDT_DStar_dstar_PbPb_MC_wEvtplane_Aug22.root (new ntupler, D0 Dau track cut 1 GeV , slow pion pT cut 0p3)
// - /home/jun502s/DstarAna/DStarAnalysis/Macro/skim/Data/FlatSample/ppMC/DStar/0818/flatSkimForBDT_DStar_Mix_PPRef_MC_Aug18.root

void FlatEffAccCalculator_PbPb(
    const std::string& datasetKey = "PbPb_Data_Aug25",
    const std::string& datasetCatalog = "/home/jun502s/DstarAna/DStarAnalysis/Data/datasets.json",
    // string fileMCNP,
    
    string subDir = SelectionCuts::SubDir(),
    bool cutBased = false, 
    int mvaV = 0,
   ...
   // double mvaCut = selMVA.low;
    ...
    std::unique_ptr<TFile> datasetFile(
        LoadDatasetFromJSON(datasetKey.c_str(), datasetCatalog.c_str()));
    if (!datasetFile) {
        std::cerr << "Error loading dataset: " << datasetKey
                  << " from catalog " << datasetCatalog << std::endl;
        return;
    }

    TFile* fMCPR = datasetFile.get();
    ...
    // Define base directory for all outputs under Corrections/Data/PbPb/<subDir>
    // Relative to this source's folder (Corrections/Generators/PbPb), "../../Data/PbPb/<subDir>"
    const std::string baseDir = Form("../../Data/PbPb/%s", subDir.c_str());
    createDir(baseDir.c_str());

    TFile* outputFile = new TFile(Form("%s/output.root", baseDir.c_str()), "recreate");
    ...
    auto fillReco = [&](double dzW){
		...
		evtRecoFlat.mva > 0.99 &&
                ...
                // TVector3 vect = DstarDau1Vector_EventPlane(Dstar, D0, evtRecoFlat.Psi2Raw_Trk);
                TVector3 vect = DstarDau1Vector_Helicity(Dstar, D0);
    ...
    // ratio3DPR->Draw("box2");
    c1->cd(1);
    ratioPtPR->GetYaxis()->SetRangeUser(0, std::max(ratioPtPR->GetMaximum(), ratioPtNP->GetMaximum()));
    ratioPtPR->Draw();
    ratioPtNP->Draw("same");
    c1->cd(2);
    ratioYPR->GetYaxis()->SetRangeUser(0, std::max(ratioYPR->GetMaximum(), ratioYNP->GetMaximum()));
    ratioYPR->Draw();
    ratioYNP->Draw("same");
    c1->cd(3);
    ratioPhiPR->GetYaxis()->SetRangeUser(0, std::max(ratioPhiPR->GetMaximum(), ratioPhiNP->GetMaximum()));
    ratioPhiPR->Draw();
    ratioPhiNP->Draw("same");
    c1->cd(4);
    ratioCosPR->GetYaxis()->SetRangeUser(0, std::max(ratioCosPR->GetMaximum(), ratioCosNP->GetMaximum()));
    ratioCosPR->Draw();
    ...
    c1->SaveAs(Form("%s/distributions.pdf", baseDir.c_str()));
    c1->SaveAs(Form("%s/distributions.png", baseDir.c_str()));
...
// c_proj->SaveAs(Form("%s/projCos_firstPtBin.png", baseDir.c_str()));
    ratio3DPR->Write();
    ratio3DNP->Write();
    ratio3DPRACC->Write();
    ratio3DNPACC->Write();
