...
private:
    ...
    bool DatasetHasCosVariable() const;
...
EnhancedPlotManager::EnhancedPlotManager(const FitOpt& opt, const std::string& inputDir, const std::string& outputFile, 
    ...
    if (opt_.drawMvaLegend && !opt_.mvaLegend.empty()) {
        std::cout << "  MVA: " << opt_.mvaLegend << std::endl;
    }
...
std::string EnhancedPlotManager::GetOutputFileName(const std::string& baseName, const std::string& suffix) {
    return PlotFileHelper::BuildFilename(outputDir_, baseName, suffix);
}
...
bool EnhancedPlotManager::DatasetHasCosVariable() const {
    if (!dataset_) return false;
    auto* args = dataset_->get();
    if (!args) return false;
    return args->find("cosThetaEP") || args->find("cosThetaHX") || args->find("cosThetaCS");
}
...
void EnhancedPlotManager::AddLabels(TPad* pad) {
    ...
    if (!opt_.cosLegend.empty() && DatasetHasCosVariable()) {
    ...
    if (opt_.drawMvaLegend && !opt_.mvaLegend.empty()) {
        cmsLabel->DrawLatex(0.65, yPos, opt_.mvaLegend.c_str());
        yPos -= 0.05;
    }
...
void EnhancedPlotManager::PrintSummary() const {
    ...
    if (opt_.drawMvaLegend && !opt_.mvaLegend.empty()) {
        std::cout << "MVA cut: " << opt_.mvaLegend << std::endl;
    }
...
bool EnhancedPlotManager::DrawRawDistribution(const std::string& outputName) {
    ...
    // Save plot
    if (!PlotFileHelper::EnsureDirectory(outputDir_)) {
...
bool EnhancedPlotManager::DrawFittedModel(bool drawPull, const std::string& outputName, bool drawParameterPad) {
    ...
    if (opt_.drawMvaLegend && !opt_.mvaLegend.empty()) {
        leftLabels.push_back(opt_.mvaLegend);
    }
    if (opt_.drawDcaLegend && !opt_.dcaLegend.empty()) {
