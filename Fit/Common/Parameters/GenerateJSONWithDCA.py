...
def main() -> None:
    ...
    parser.add_argument("--case", choices=["PP", "PbPb"], default="PP",
                        help="Analysis case: PP or PbPb (affects cos bins and output directory)")
    ...
    # Set default cos bins and centrality bins based on case
    if args.cos_bins is None:
        # Both PP and PbPb use the same symmetric cos bins
        cos_bin_specs = ["-1.0:-0.8", "-0.8:-0.6", "-0.6:-0.4", "-0.4:-0.2", "-0.2:0.0", 
                        "0.0:0.2", "0.2:0.4", "0.4:0.6", "0.6:0.8", "0.8:1.0"]
    else:
        cos_bin_specs = [item for group in args.cos_bins for item in group]
    cos_bins = parse_ranges(cos_bin_specs, float)
    
    # Set default centrality bins based on case
    if args.cent_bins == ["0:10"]:  # Only if using default
        if args.case == "PP":
            cent_bins = parse_ranges(["0:100"], int)  # PP: single centrality bin
        else:  # PbPb
            cent_bins = parse_ranges(["0:10", "10:50"], int)  # PbPb: two centrality bins
    else:
    ...
    # Set output path based on case and args
    import os
    if args.output:
        ...
        case_dir = f"{args.case}"
        if not os.path.exists(case_dir):
            os.makedirs(case_dir)
        out_path = os.path.join(case_dir, f"dstar_parameters_{signal_type}_{background_type_requested}_dca_{args.case.lower()}.json")
