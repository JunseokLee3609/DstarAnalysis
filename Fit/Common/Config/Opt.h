...
struct FitOpt : public FitCommonSettings {
    ...
    std::string MCdatasetName;      // MC dataset name for variable selection
    std::string cutExpr;            
    ...
    // Generate MC cut expression with dataset-specific cos variable
    std::string GetMCCutString(bool useAbsCos = false) const {
        std::string cuts = BuildKinematicCutForMC(GetKinematicBin(), useAbsCos);
        if (mvaMin > 0.0) {
            cuts += " && " + mvaVar + " > " + std::to_string(mvaMin);
        }
        if (!cutMCExpr.empty()) {
    ...
    void GenerateLegends() {
        ...
        if (useIntegratedMC) {
            ...
            if (cosMin > -2.0 || cosMax < 2.0) {
                // cos: 소수점 둘째 자리까지 (%.2f)
                // 데이터셋에 따라 cos 변수명 결정
                std::string cosVarName = "cos#theta*";  // 기본값
                if (datasetName == "datasetEP") {
                    cosVarName = "cos#theta_{EP}*";
                } else if (datasetName == "datasetHX") {
                    cosVarName = "cos#theta_{HX}*";
                }
                cosLegend = Form("%.2f < %s < %.2f", cosMin, cosVarName.c_str(), cosMax);
            } else {
                // 데이터셋에 따라 cos 변수명 결정
                std::string cosVarName = "cos#theta*";  // 기본값
                if (datasetName == "datasetEP") {
                ...
                cosLegend = Form("Full %s range", cosVarName.c_str());
            }
        }
...
public:
    ...
    std::string BuildKinematicCut(const KinematicBin& bin, bool useAbsCos = false) const {
        ...
        if (bin.HasCosRestriction()) {
            // Data dataset 이름에 따라 cos 변수 선택
            std::string dataCosVar = cosVar; // 기본값
            if (datasetName == "datasetEP") {
                dataCosVar = "cosThetaEP";
            }
            std::string cosExpr = useAbsCos ? "abs(" + dataCosVar + ")" : dataCosVar;
    ...
    std::string BuildKinematicCutForMC(const KinematicBin& bin, bool useAbsCos = false) const {
        std::ostringstream cuts;
        cuts << ptVar << " > " << bin.pTMin << " && " << ptVar << " < " << bin.pTMax;

        if (bin.HasCosRestriction()) {
            // MC dataset에 따라 cos 변수 결정
            std::string mcCosVar = cosVar;  // 기본값
            if (MCdatasetName == "datasetEP") {
                mcCosVar = "cosThetaEP";
            }
            
            std::string cosExpr = useAbsCos ? "abs(" + mcCosVar + ")" : mcCosVar;
            cuts << " && " << cosExpr << " > " << bin.cosMin
                 << " && " << cosExpr << " < " << bin.cosMax;
        }
