...
inline std::unique_ptr<RooFitResult> ConstraintFitStrategy::Execute(RooAbsPdf* pdf, RooDataSet* data, const FitOpt& config) {
    ...
    if (constraintParameters_.empty() && !config.constraint.parameters.empty()) {
        constraintParameters_ = config.constraint.parameters;
    }
    if (mcFilePath_.empty()) {
        mcFilePath_ = config.constraint.HasMCResultFile()
            ? config.constraint.mcResultFile
...
public:
    ...
    std::unique_ptr<RooFitResult> Execute(RooAbsPdf* pdf, RooDataSet* data, const FitOpt& config, RooRealVar* massVar) override {
        ...
        const auto& params = paramNames_.empty() ? config.constraint.parameters : paramNames_;
        const double sigmaScale = (sigmaScale_ > 0) ? sigmaScale_ : config.constraint.gaussianSigmaScale;
        ...
        if (!mcResultFile_.empty()) {
            ...
            for (const auto& name : params) {
                ...
                if (auto* v = dynamic_cast<RooRealVar*>(arg)) {
                    ...
                    constraints[name] = {mu, std::max(err * sigmaScale, 1e-6)};
                }
            }
        } else {
        ...
        std::cout << "[GC][Debug] Prepared " << constraints.size() 
                  << " constraints (sigmaScale=" << sigmaScale << ")" << std::endl;
...
public:
    ...
    std::unique_ptr<RooFitResult> Execute(RooAbsPdf* pdf, RooDataSet* data, const FitOpt& config, RooRealVar* massVar) override {
        ...
        const std::string effectiveFile = mcResultFile_.empty()
            ? config.constraint.mcResultFile
            : mcResultFile_;
        if (effectiveFile.empty()) {
            std::cerr << "[GC-File] ERROR: MC result file path is empty" << std::endl;
            return nullptr;
        }
        ...
        std::unique_ptr<TFile> fin(TFile::Open(effectiveFile.c_str(), "READ"));
        if (!fin || fin->IsZombie()) {
            std::cerr << "[GC-File] ERROR: Cannot open MC result file '" << effectiveFile << "'" << std::endl;
            return nullptr;
        }
...
public:
    ...
    std::unique_ptr<RooFitResult> Execute(RooAbsPdf* pdf, RooDataSet* data, const FitOpt& config, RooRealVar* massVar) override {
        ...
        const auto& sbCfg = config.constraint.sideband;
        const double loMin = (sbLoMin_ < sbLoMax_) ? sbLoMin_ : sbCfg.lowMin;
        const double loMax = (sbLoMin_ < sbLoMax_) ? sbLoMax_ : sbCfg.lowMax;
        const double hiMin = (sbHiMin_ < sbHiMax_) ? sbHiMin_ : sbCfg.highMin;
        const double hiMax = (sbHiMin_ < sbHiMax_) ? sbHiMax_ : sbCfg.highMax;
        const double effSigma = (sigmaScale_ > 0) ? sigmaScale_ : sbCfg.sigmaScale;

        // Define sideband ranges on the mass variable
        massVar->setRange("SBlo", loMin, loMax);
        massVar->setRange("SBhi", hiMin, hiMax);
        ...
        std::cout << "[SB] Prefitting background in sidebands [" << loMin << ", " << loMax
                  << "] U [" << hiMin << ", " << hiMax << "]" << std::endl;
...
public:
    ...
    std::unique_ptr<RooFitResult> Execute(RooAbsPdf* pdf, RooDataSet* data, const FitOpt& config, RooRealVar* massVar) override {
        ...
        const auto& params = paramNames_.empty() ? config.constraint.parameters : paramNames_;
        const double sigmaSig = (sigmaScaleSignal_ > 0) ? sigmaScaleSignal_ : config.constraint.signalSigmaScale;
        const double sigmaBkg = (sigmaScaleBkg_ > 0) ? sigmaScaleBkg_ : config.constraint.backgroundSigmaScale;
        const auto& sbCfg = config.constraint.sideband;
        const double loMin = (sbLoMin_ < sbLoMax_) ? sbLoMin_ : sbCfg.lowMin;
        const double loMax = (sbLoMin_ < sbLoMax_) ? sbLoMax_ : sbCfg.lowMax;
        const double hiMin = (sbHiMin_ < sbHiMax_) ? sbHiMin_ : sbCfg.highMin;
        ...
        std::map<std::string, std::pair<double,double>> sigConstr;
        {
            ...
            for (const auto& name : params) {
                if (auto* v = dynamic_cast<RooRealVar*>(sigVars->find(name.c_str()))) {
                    ...
                    if (err <= 0) err = 1e-6;
                    sigConstr[name] = {mu, std::max(err * sigmaSig, 1e-6)};
                }
            }
        }
        ...
        if (!m) return nullptr;
        m->setRange("SBlo", loMin, loMax);
        m->setRange("SBhi", hiMin, hiMax);
        {
            RooLinkedList preOpts;
            preOpts.Add(new RooCmdArg(RooFit::NumCPU(config.numCPU)));
            preOpts.Add(new RooCmdArg(RooFit::PrintLevel(config.verbose ? 1 : -1)));
            ...
            for (auto it = bkgVars->fwdIterator(); auto* var = it.next();) {
                if (auto* v = dynamic_cast<RooRealVar*>(var)) {
                    ...
                    if (err <= 0) err = 1e-6;
                    bkgConstr[pname] = {mu, std::max(err * sigmaBkg, 1e-6)};
                }
            }
        }
...
inline std::unique_ptr<FitStrategy> FitStrategyFactory::CreateGaussianConstraintFromFileStrategy(
    const FitOpt& config,
    const std::string& resultObjName = "fitResult") {
    return CreateGaussianConstraintFromFileStrategy(
        config.constraint.mcResultFile,
        config.constraint.parameters,
        resultObjName,
        config.constraint.gaussianSigmaScale);
}
...
inline std::unique_ptr<FitStrategy> FitStrategyFactory::CreateSidebandPrefitBackgroundConstraintStrategy(
    RooAbsPdf* backgroundPdf,
    const FitOpt& config) {
    const auto& sb = config.constraint.sideband;
    return CreateSidebandPrefitBackgroundConstraintStrategy(
        backgroundPdf,
        sb.lowMin, sb.lowMax,
        sb.highMin, sb.highMax,
        sb.sigmaScale);
}
...
inline std::unique_ptr<FitStrategy> FitStrategyFactory::CreateConstraintStrategy(const FitOpt& config) {
    const auto& constraint = config.constraint;
    const std::string path = constraint.HasMCResultFile()
        ? constraint.mcResultFile
        : (config.outputDir + "/" + config.outputFile);
    return CreateConstraintStrategy(path, constraint.parameters);
}
...
inline std::unique_ptr<FitStrategy> FitStrategyFactory::CreateGaussianConstraintStrategy(
    ...
    RooAbsPdf* mcSignalPdf,
    const FitOpt& config) {
    return CreateGaussianConstraintStrategy(mcDataset,
                                            mcSignalPdf,
                                            config.constraint.parameters,
                                            config.constraint.gaussianSigmaScale);
}
...
inline std::unique_ptr<FitStrategy> FitStrategyFactory::CreateGaussianConstraintWithSBStrategy(
    ...
    const FitOpt& config) {
    const auto& constraint = config.constraint;
    const auto& sb = constraint.sideband;
    return CreateGaussianConstraintWithSBStrategy(
        mcDataset,
        mcSignalPdf,
        backgroundPdf,
        ...
        sb.lowMin, sb.lowMax,
        sb.highMin, sb.highMax,
        constraint.signalSigmaScale,
        constraint.backgroundSigmaScale);
}
