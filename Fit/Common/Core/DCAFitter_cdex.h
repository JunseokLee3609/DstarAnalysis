...
#include "../Core/MassFitterV2.h"  // Updated to use V2
#include "../Utils/FitStrategy.h"    // For configurable fitting strategies
#include "../Utils/JSONParameterUtils.h"    // For loading PDF params from JSON
#include "../Utils/JSONParameterLoader.h"   // Loader interface
// #include "../Tools/ConfigManager.h"  // Optional external configuration manager
#include "../Managers/ErrorHandler.h"   // For enhanced error handling (ConsoleErrorHandler, etc.)
                            ...
                            jsonParams.ApplyToSignalParams([&](const auto& sigParams) {
                                jsonParams.ApplyToBackgroundParams([&](const auto& bkgParams) {
                                    ...
                                    if (dcaSliceMC) {
                                        ...
                                        FitOpt gcOpt = massFitOpt;
                                        gcOpt.constraint.sideband.lowMin = sbLoMin;
                                        gcOpt.constraint.sideband.lowMax = sbLoMax;
                                        gcOpt.constraint.sideband.highMin = sbHiMin;
                                        gcOpt.constraint.sideband.highMax = sbHiMax;
                                        gcOpt.constraint.sideband.sigmaScale = gaussianConstraintBackgroundScale_;
                                        gcOpt.constraint.signalSigmaScale = gaussianConstraintSignalScale_;
                                        gcOpt.constraint.backgroundSigmaScale = gaussianConstraintBackgroundScale_;
                                        if (!paramsToConstrain.empty()) {
                                            gcOpt.constraint.parameters = paramsToConstrain;
                                        }
                                        massFitter->SetConfiguration(gcOpt);
                                        fitSuccess = massFitter->PerformGaussianConstraintFitWithMCAndBkgSB(
                                            dcaSliceData, dcaSliceMC,
                                            sigParams, bkgParams,
                                            paramsToConstrain,
                                            sliceName);
