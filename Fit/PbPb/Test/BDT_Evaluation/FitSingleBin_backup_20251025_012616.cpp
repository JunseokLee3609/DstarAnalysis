...
void FitSingleBin(int kinVarInt, double varMin, double varMax, int generateHistograms = 1, int doFit = 1) {
    ...
    for (size_t mvaIdx = 0; mvaIdx < mvaThresholds.size(); ++mvaIdx) {
        ...
        // MC fit: signal-only fit to extract tail parameters
        ...
        if (bkgType == "Phenomenological2") {
            ...
            RooRealVar m_pi_param("m_pi_param", "m_pi", 0.13957);
            m_pi_param.setConstant(true);
            
            // Use m_param as m and c_param as lambda
            ...
            // Formula: (x - m0)^m * exp(lambda * (x - m0))
            RooArgList phenom2Args(massPion, m_pi_param, m_param, c_param);
        ...
        RooFitResult* dataFitRes = model.fitTo(dataBinnedData, Save(), Extended(kTRUE), SumW2Error(kFALSE), PrintLevel(-1), Warnings(kFALSE), Strategy(1));
        ...
        model.plotOn(dataFrame, Components("signalPdf"), LineStyle(kDashed), LineColor(kRed), LineWidth(2), Name("signal"));
        ...
        model.plotOn(dataFramePlot, Components("signalPdf"), LineStyle(kDashed), LineColor(kRed), LineWidth(2), Name("signal"));
        ...
        if (mcFrame && mcFitResForPlot && dataFramePlot) {
            ...
            legData->AddEntry(dataFramePlot->findObject("data"), "Data", "lep");
            legData->AddEntry(dataFramePlot->findObject("model"), "Fit", "l");
            legData->AddEntry(dataFramePlot->findObject("signal"), "Signal", "l");
            legData->AddEntry(dataFramePlot->findObject("bkg"), "Background", "l");
            legData->Draw();
            padData->Update();
