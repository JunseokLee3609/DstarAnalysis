#!/bin/bash
# Submit Condor jobs for MakeHistKinematicBin.cpp at MVA 0.990 and 0.999 (combined kinematic bins)
set -e
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
cd "$SCRIPT_DIR"

BINS_FILE="kinematic_bins_queue.txt"
if [[ ! -f "$BINS_FILE" ]]; then echo "Error: $BINS_FILE not found"; exit 1; fi

MVA_LIST=("0.990" "0.999")
...
while read -r ptmin ptmax yamin yamax cmin cmax; do
  [[ "$ptmin" =~ ^# ]] && continue
  [[ -z "$ptmin" ]] && continue
  for mva in "${MVA_LIST[@]}"; do
    {
      echo "transfer_input_files = run_make_hist_bin.sh, MakeHistKinematicBin.cpp"
      echo "arguments = \"$ptmin $ptmax $yamin $yamax $cmin $cmax $mva\""
      echo "queue"
      echo ""
    } >> "$CONDOR_FILE"
  done
