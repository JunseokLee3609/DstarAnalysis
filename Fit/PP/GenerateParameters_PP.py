#!/usr/bin/env python3
"""Generate D* JSON parameter grids for PP analysis."""
...
def make_signal_params_dbcb() -> Dict[str, Dict[str, object]]:
    """PP signal parameters - Double-sided Crystal Ball"""
    return {
...
def make_background_params_phenom2() -> Dict[str, Dict[str, object]]:
    """PP background parameters - Phenomenological2"""
    return {
...
def build_pp_grid(pt_ranges: List[Tuple[float, float]]) -> Dict[str, dict]:
    """Build pT grid for PP analysis"""
    grid = {}
    ...
    for pt_min, pt_max in pt_ranges:
        bin_id = f"pT_{pt_min:.1f}_{pt_max:.1f}"
        grid[bin_id] = {
            "kinematic": {
                "pT_min": pt_min,
                "pT_max": pt_max,
...
def main():
    parser = argparse.ArgumentParser(description="Generate PP parameter grid")
    parser.add_argument(
        "--pt-bins",
    ...
    parser.add_argument(
        "-o", "--output",
        default="Parameters/dstar_parameters_DBCrystalBall_Phenomenological2_PP.json",
    ...
    # Build grid
    grid = build_pp_grid(pt_ranges)
    
    # Save to JSON
    with open(args.output, "w") as f:
        json.dump(grid, f, indent=2)
    
    print(f"✓ Generated PP parameters: {args.output}")
